...
class InputEmbedding(nn.Module):
    ...
    def __init__(self, d_model: int, vocab_size: int): # d_model: size of the embedding vector (1 word)
        super().__init__()
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        # Apply the sin to even positions, the cos to odd positions
    ...
    def forward(self, x):
        x = x + (self.pe[:, :x.shape[1], :]).requires_grad_(False) # Fixed, NOT learning during both training and inference
...
class ProjectionLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.log_softmax(self.proj(x), dim = -1) # LogSoftmax for the stability
